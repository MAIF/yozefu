...
impl Default for Highlighter {
    fn default() -> Self {
        Self::new(THEMES.themes[HIGHLIGHTER_DEFAULT_THEME].clone())
    }
}

impl<'a> Highlighter {
    /// `name` and `fallback` could be a absolute file path to a `.tmTheme` file or a syntect theme name.
    pub fn theme(name: &Option<String>, fallback: &str) -> highlighting::Theme {
        let name = name.as_deref().unwrap_or(fallback);
        Self::try_to_load(name)
            .or(Self::try_to_load(fallback))
            .unwrap_or_else(|| {
                warn!("Warning: Theme '{name}' not found, falling back to '{HIGHLIGHTER_DEFAULT_THEME}'");
                THEMES.themes.get(HIGHLIGHTER_DEFAULT_THEME).expect("ddd").clone()
            })
            .clone()
    }

    pub fn try_to_load(name: &str) -> Option<Theme> {
